...
import { Avatar, Button, Card, Checkbox, Icon, Input, Layout, List } from 'antd';
...
export const Video= (props) => {
  ...
  const toBase64 = (arr) => {
    let video = new Buffer.from(arr).toString("base64");
    return video;
  }
  const render = () => {
    const result = [];
    ...
    const callNftData = async() => {
      try {
        ...
        await axios.post(apiUrl, {
          ...
          let videoList = imageFileList.filter(res => !(res.dataExt === 'png'
                                                    || res.dataExt === 'jpeg'
                                                    || res.dataExt === 'jpg'
                                                    || res.dataExt === 'bmp'
                                                    || res.dataExt === 'svg'));
          setVideoNumber(videoList.length);
          setImageNumber(imageFileList.length - videoList.length);
                  ...
                  <div style={{position: '-webkit-sticky', position: 'sticky',top: '0px', height: '450px' , marginTop: '5%', textAlign: 'center', width: '100%'}}>
                    ...
                    <div style={{marginTop:'25px', width:'100%'}}>
                      <div style={{fontSize:'45px'}}>
                        <div style={{color: "#2a3434", verticalAlign:'center', display:'inline-block'}}>  <p style={{display:'inline-block', float:'left'}}>@{cookies.user_Id}  </p>
                      ...
                      <div style={{color: "#2a3434", fontWeight:"bold", fontSize: '20px', textAlign: 'left', marginLeft: '15px'}}>
                        Followed By
                      </div>
                      ...
                      <div style={{color: '#a5a7a7', textAlign: 'left', fontSize:'20px', marginLeft: '15px', fontWeight: 'bold', cursor: 'pointer', marginBottom: '10px', marginTop:'50px'}}> View All </div>
                        ...
                        <div style={{textAlign: 'left'}}>
                          <h2>Token <span> {userTkn} NHC</span></h2> 
                      ...
                      renderItem={item=> (
                        <Link to ={{
                          ...
                          <List.Item className="card_object">
                            <Card
                              cover={
                                <video src={"data:video/" + item.dataExt + ";base64," + toBase64(item.dataBuffer)} alt="NFT" height="300px" width="200px" autoPlay loop />
                              }
                            ...
                            <div className="purchaseinfo" >
                              <div className="information"> 
                                <span className="bid"> Token Price </span>
                              </div>
